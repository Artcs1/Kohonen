...
def erro_quantização(model,X):
    ...
    for i in np.arange(tam):
        Xi = X[i,]
        BMU = np.min(dist_euclidian(Xi,model[2]))
        E = E + BMU
        
    return E/tam

def erro_topologico(model,I,X,Y ):
    
    c = 0
    for i in np.arange(X.shape[0]):
        BMU = np.argmin(dist_euclidian(X[i,],model[2]))
        if I[BMU//10][BMU%10] == Y[i]:
            c= c +1;
    return c/(model)
...
def main():   
   ...
   print("Erro de quantização : " + str(erro_quantização(trained,X)))
